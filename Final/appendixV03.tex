...
\noindent\textit{Proof:}  %define the projection of $\MMedian$ into $SO(3)$ as $\ProjMedian^*$, i.e. $\ProjMedian^*=\argmin_{\bm S\in SO(3)}||\MMedian-\bm S||_F$ and suppose that  $\ProjMedian^*\neq\ProjMedian$.
...
&= n||\MMedian-\ProjMedian^*||_F-n||\MMedian-\ProjMedian||_F\nonumber \le 0 \text{ by definition of } \ProjMedian^*\\
\implies&||\MMedian-\ProjMedian^*||_F = ||\MMedian-\ProjMedian||_F
\end{align}
%In turn \eqref{eq:contradiction} implies $||\MMedian-\ProjMedian||_F<||\MMedian-\ProjMedian^*||_F$, which is a contradiction by the definition of $\ProjMedian^*$.  
The result follows. \hh{but only if the projection of $\MMedian$ onto SO(3) is unique - which is only true, if $\MMedian$ is not  in the center of the sphere.}
