...
In Figure~\ref{fig:Haar}(a)-(b), we see that the circular-von Mises-based distribution has the most mass around the mode of zero whereas the Cayley distribution has the least mass around its mode.  \red{Note that even though these densities look like they have vastly different variances their circular variances are the same when evaluated with respect to the Haar measure.}    Figure \ref{fig:Haar}(c) offers a better view of the tail behavior of the distributions.  A visualization of a random sample of 100 rotations, one sample for each of the three angular distributions, is given in the sphere plots in Figure~\ref{eyeballs}. The samples are adjusted to have an circular variance of $\nu = 0.25$.  Note that Figure~\ref{eyeballs} shows only the first of the three columns of each rotation matrix. We refer to the supplemental material online for a complete visualization of the three samples.
