...
A visualization of a random sample of 100 rotations, one sample for each of the three angular distributions, is given in the sphere plots in Figure~\ref{eyeballs}. The concentration parameter $\kappa$ for each distribution is chosen such that the distributions have an angular variance of $\nu = 0.25$. Note that Figure~\ref{eyeballs} shows only the first of the three columns of each rotation matrix. %, because, however rotation matrices are orthogonal with each column and row representing one element on the unit sphere,  Under the assumption of independence of the angle $r$ and the main direction, any of the other directions is expected to show a similar pattern. 
...
%\red{each rotation matrix, because, however rotation matrices are orthogonal with each column and row representing one element on the unit sphere,  Under the assumption of independence of the angle $r$ and the main direction, any of the other directions is expected to show a similar pattern. Figure~\ref{eyeballs} also confirms the previous findings from Figure~\ref{fig:Haar}; the circular-von Mises-based distribution has the highest mass in close proximity around the main direction but at the same time shows the largest numbers of `outlying' rotations, i.e.~rotations with a large radial distance to the center, while the Cayley and the matrix-Fisher distribution appear to be more similar with a lower concentration of mass in the center for the Cayley distribution than for the matrix-Fisher distribution. }
>>>>>>> shorteninh eyeballe explanation
