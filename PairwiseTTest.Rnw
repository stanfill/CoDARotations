...
<<badShatR,echo=FALSE,fig.align='center',out.width="1\\linewidth",fig.height=7,fig.width=7,fig.cap="Plot of the difference in errors for the mean-estimators versus the error of the geometric mean, $\\widehat{S}_R$.  It seems reasonable that the points off of the big mass for n=10 are victims of bad starting points.">>=
ResSub<-Res[Res$nu==.75 & Res$n%in%c(10,50) & Res$Dist=="Cayley",c(3,6,12:17)]
qplot(Means,ML2Error,data=ResSub,facets=.~n,ylab=expression(hat(S)[R]),xlab=expression(hat(S)[R]-hat(S)[E]))
@
<<baddiehist,echo=FALSE,fig.align='center',out.width="1\\linewidth",fig.height=7,fig.width=7,fig.cap="Histogram of misorientation angles that led to a bad estimate with $\\widehat{S}_R$">>=
