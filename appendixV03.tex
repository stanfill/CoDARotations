...
d_E(\bm{R}_1,\bm{R}_2)=2^{3/2}\sin\left(\frac{d_G(\bm{R}_1,\bm{R}_2)}{2}\right).
\end{equation}
\emph{Proof:}\\
Let $\bm{o}_1,\bm{R}_2\in SO(3)$ be given and define $\tr(\bm R_1^\top\bm R_2)=1+2\cos(\theta)$ then $|\theta|=d_G(\bm R_1,\bm R_2)$.  Notice that \eqref{EvR} holds trivially for $d_E(\bm R_1,\bm R_2)=d_G(\bm R_1,\bm R_2)=0$ so consider the case $|\theta|>0$.  By definition of $d_E(\bm R_1,\bm R_2)$ we have the following:
\begin{align*}
d_E(\bm R_1,\bm R_2)^2
&=||\bm R_1-\bm R_2||_F^2\\
...
&=\left[2^{3/2}\sin\left(\frac{d_G(\bm R_1,\bm R_2)}{2}\right)\right]^2
\end{align*}
Taking square roots on both sides and noticing that $d_E(\bm R_1,\bm R_2)\geq0$ and $d_G(\bm R_1,\bm R_2)\geq0$ gives \eqref{EvR}.\\
$\Box$
